...
export const getSecondaryDiagonal = (board: Board, rowIndex: number, colIndex: number, boardSize: number): Cell[] => {
   return rowIndex + colIndex === boardSize - 1 ? board.map((row, i) => row[boardSize - (i + 1)]) : []
}
